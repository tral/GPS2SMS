...
class DBHelper extends SQLiteOpenHelper {
	...
	public static int getRndColor() {
		...
		// Чтобы не генерился слишком светлый
		// фон, иначе символы нечитаемы
	...
	public void onCreate(SQLiteDatabase db) {
		...
		// Договорились, что телефон хранится в
		// таблице с _id=1
		cv.put("_id", 1);
		cv.put("phone", ""); // без "+7" !!!
		...
		// Договорились, что хранится в таблице
		// с _id=1
		cv.clear();
		cv.put("_id", 1);
		cv.put("contact", "");
	...
	public void Upgrade_2_to_3(SQLiteDatabase db) {
		...
		cv.clear();
		cv.put("param", "keepscreen"); // Держать ли экран
										// всегда включенным
		cv.put("val_txt", "");
		cv.put("val_int", 1);
		db.insert("settings", null, cv);
